...
//没有内容时默认高度
CGFloat defaultHeight = 36.0f;
//elementKind
NSString *decorationViewOfKind = @"decorationBgView";

@interface AJSectionBackgroundLayout()
@property (strong, nonatomic) NSMutableArray *itemAttributes;
@end

@implementation AJSectionBackgroundLayout
...
- (void)prepareLayout {
    ...
    for (int section = 0; section < numberOfSection-1; section++) {
        ...
        attributes.zIndex = -1;
        if (lastIndex < 0) {
            frame = CGRectMake(0, 30, self.collectionView.frame.size.width, 36);
            if (section == 1) {
        ...
        [self registerClass:[AJSectionBackgroundView class] forDecorationViewOfKind:decorationViewOfKind];
    }
}
